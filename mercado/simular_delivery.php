...
$order_id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT) ?: 30;
if ($order_id <= 0) {
    $order_id = 30;
}
...
// Buscar pedido
try {
    $stmt = $pdo->prepare("SELECT order_id, delivery_id, shipping_lat, shipping_lng FROM om_market_orders WHERE order_id = ?");
    ...
    die('Erro ao carregar dados do pedido');
}

// Buscar delivery
$delivery = null;
if ($order && $order['delivery_id']) {
    $stmt = $pdo->prepare("SELECT delivery_id, name, lat, lng FROM om_market_deliveries WHERE delivery_id = ?");
    $stmt->execute([$order['delivery_id']]);
    $delivery = $stmt->fetch(PDO::FETCH_ASSOC);
}

$dest_lat = (float)($order['shipping_lat'] ?? -23.5505);
$dest_lng = (float)($order['shipping_lng'] ?? -46.6333);
$delivery_lat = (float)($delivery['lat'] ?? -23.5415);
$delivery_lng = (float)($delivery['lng'] ?? -46.6333);
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn { display: block; width: 100%; padding: 16px; border: none; border-radius: 10px; font-size: 16px; font-weight: 600; cursor: pointer; margin-bottom: 12px; }
        .btn-orange { background: #f97316; color: white; }
        .btn-green { background: #10b981; color: white; }
        .btn-blue { background: #3b82f6; color: white; }
        .btn:hover { opacity: 0.9; }
        .btn:active { transform: scale(0.98); }
        .info { font-size: 13px; color: #94a3b8; margin-bottom: 8px; }
        .coords { font-family: monospace; font-size: 12px; background: #0f172a; padding: 10px; border-radius: 8px; margin-top: 12px; }
        .log { background: #0f172a; padding: 12px; border-radius: 8px; font-size: 12px; font-family: monospace; max-height: 200px; overflow-y: auto; margin-top: 12px; }
        .log-item { padding: 4px 0; border-bottom: 1px solid #1e293b; }
        .progress { height: 8px; background: #334155; border-radius: 4px; margin-top: 12px; overflow: hidden; }
        .progress-bar { height: 100%; background: linear-gradient(90deg, #f97316, #10b981); transition: width 0.3s; }
...
<body>
<div class="container">
    <h1>🚴 Simular Movimento</h1>
    
    <div class="card">
        <p class="info">Pedido #<?= htmlspecialchars($order_id, ENT_QUOTES, 'UTF-8') ?> | Delivery: <?= htmlspecialchars($delivery['name'] ?? 'N/A', ENT_QUOTES, 'UTF-8') ?></p>
        
        <button class="btn btn-orange" onclick="mover()">
            🚴 Mover Delivery (aproximar 20%)
        </button>
        
        <button class="btn btn-green" onclick="moverAuto()">
            ▶️ Mover Automático (a cada 3s)
        </button>
        
        <button class="btn btn-blue" onclick="resetar()">
            🔄 Resetar Posição
        </button>
        ...
        <div class="coords" id="coords">
            📍 Delivery: <?= $delivery_lat ?>, <?= $delivery_lng ?><br>
            🏠 Destino: <?= $dest_lat ?>, <?= $dest_lng ?>
        </div>
        
        <div class="log" id="log">
            <div class="log-item">Pronto para simular...</div>
        </div>
    </div>
    
    <div class="card">
        <p class="info">Abra o tracking em outra aba para ver o mapa atualizar:</p>
        <a href="tracking_v2.php?id=<?= $order_id ?>" target="_blank" class="btn btn-blue" style="text-align:center;">
            🗺️ Abrir Tracking (nova aba)
        </a>
    </div>
</div>
...
const DELIVERY_ID = <?= (int)($delivery['delivery_id'] ?? 0) ?>;
const DEST_LAT = <?= (float)$dest_lat ?>;
const DEST_LNG = <?= (float)$dest_lng ?>;

let currentLat = <?= (float)$delivery_lat ?>;
let currentLng = <?= (float)$delivery_lng ?>;
let autoInterval = null;

function log(msg) {
    const el = document.getElementById('log');
    const time = new Date().toLocaleTimeString('pt-BR', {hour:'2-digit', minute:'2-digit', second:'2-digit'});
    el.innerHTML = `<div class="log-item">[${time}] ${msg}</div>` + el.innerHTML;
}
...
function updateCoords() {
    ...
    updateTimeout = setTimeout(() => {
        ...
        coordsEl.innerHTML = `
            📍 Delivery: ${currentLat.toFixed(6)}, ${currentLng.toFixed(6)}<br>
            🏠 Destino: ${DEST_LAT}, ${DEST_LNG}
        `;
        
        // Calcular progresso
        const totalDist = Math.sqrt(Math.pow(-23.5415 - DEST_LAT, 2) + Math.pow(-46.6333 - DEST_LNG, 2));
        const currentDist = Math.sqrt(Math.pow(currentLat - DEST_LAT, 2) + Math.pow(currentLng - DEST_LNG, 2));
        const progress = Math.max(0, Math.min(100, (1 - currentDist / totalDist) * 100));
        progressEl.style.width = progress + '%';
    }, 16); // ~60fps
}

async function mover() {
    // Mover 20% em direção ao destino
    currentLat = currentLat + (DEST_LAT - currentLat) * 0.2;
    currentLng = currentLng + (DEST_LNG - currentLng) * 0.2;
    
    try {
        const API_BASE = document.querySelector('meta[name="api-base"]')?.content || '/api';
const res = await fetch(`${API_BASE}/delivery_gps.php`, {
            method: 'POST',
        ...
        if (data.success) {
            log(`✅ Movido! ETA: ${data.eta_minutes || '?'} min`);
            if (data.arriving) log('🎉 CHEGANDO!');
        } else {
            log('❌ Erro: ' + data.error);
        }
...
function moverAuto() {
    ...
    log('▶️ Auto iniciado (3s)');
    mover(); // Primeiro movimento imediato
    autoInterval = setInterval(mover, 3000);
}
...
async function resetar() {
    ...
    try {
        await fetch('/mercado/api/delivery_gps.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                delivery_id: DELIVERY_ID,
