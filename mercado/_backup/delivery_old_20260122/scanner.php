...
/**
 * ══════════════════════════════════════════════════════════════
 * 📱 DELIVERY - SCANNER QR CODE
 * Escaneia caixa + Confirma entrega
 * ══════════════════════════════════════════════════════════════
 */
...
$allowed_modes = ['box', 'delivery'];
$mode = in_array($_GET['mode'] ?? 'box', $allowed_modes) ? $_GET['mode'] : 'box';
$order_id = filter_input(INPUT_GET, 'order_id', FILTER_VALIDATE_INT) ?: 0;
$route_id = filter_input(INPUT_GET, 'route_id', FILTER_VALIDATE_INT) ?: 0;
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <title>Scanner - OneMundo Delivery</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
    <!-- HTML5 QR Code Scanner -->
    <script src="https://unpkg.com/html5-qrcode@2.3.8/html5-qrcode.min.js" onerror="this.onerror=null;this.src='assets/js/html5-qrcode.min.js'"></script>
    <style>
        :root {
            --bg: #030303;
            --bg-card: #0a0a0a;
            ...
            --gradient-success: linear-gradient(135deg, #00d47e 0%, #00a35f 100%);
        }
...
<body>
    ...
    <div class="header">
        ...
        <div class="header-title">
            ...
            <p id="modeLabel"><?= htmlspecialchars($mode === 'box' ? 'Escanear caixa' : 'Confirmar entrega', ENT_QUOTES, 'UTF-8') ?></p>
        </div>
    </div>

    <!-- Mode Tabs -->
    <div class="mode-tabs">
        <button class="mode-tab <?= htmlspecialchars($mode === 'box' ? 'active' : '', ENT_QUOTES, 'UTF-8') ?>" onclick="setMode('box')">
            <span class="icon">📦</span>
            Caixa
        </button>
        <button class="mode-tab <?= htmlspecialchars($mode === 'delivery' ? 'active' : '', ENT_QUOTES, 'UTF-8') ?>" onclick="setMode('delivery')">
            <span class="icon">✅</span>
            Entrega
        </button>
    </div>
    ...
    <div class="manual-section">
        <div class="divider">
            ...
            <span class="divider-text">ou digite</span>
            <div class="divider-line"></div>
        </div>
        ...
        <div class="input-group">
            <label id="inputLabel"><?= $mode === 'box' ? 'Código da Caixa' : 'Código de Entrega' ?></label>
            <input type="text" 
                   class="code-input" 
                   id="codeInput" 
                   placeholder="<?= $mode === 'box' ? 'BOX-123-ABC' : 'BANANA-123' ?>"
                   autocomplete="off"
                   oninput="this.value = this.value.toUpperCase()">
        </div>

        <button class="btn <?= $mode === 'box' ? 'btn-primary' : 'btn-success' ?>" id="validateBtn" onclick="validateCode()">
            <?= $mode === 'box' ? '📦 Confirmar Caixa' : '✅ Confirmar Entrega' ?>
        </button>
    </div>
    ...
    <div class="result-card" id="resultCard">
        ...
        <h2 class="result-title" id="resultTitle">Sucesso!</h2>
        <p class="result-message" id="resultMessage"></p>
        
        <div class="result-details" id="resultDetails"></div>
        
        <button class="btn btn-primary" id="resultBtn" onclick="handleResultAction()">
            Continuar
        </button>
    </div>

    <script>
    let currentMode = <?= json_encode($mode, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT) ?>;
    let orderId = <?= json_encode($order_id ?: null, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT) ?>;
    let routeId = <?= json_encode($route_id ?: null, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT) ?>;
    ...
    async function initScanner() {
        try {
            ...
            document.querySelector('.scanner-overlay p').textContent = 'Câmera não disponível. Digite o código manualmente.';
        }
    }
    ...
    function setMode(mode) {
        ...
        // Atualizar labels
        elements.modeLabel.textContent = mode === 'box' ? 'Escanear caixa' : 'Confirmar entrega';
        elements.inputLabel.textContent = mode === 'box' ? 'Código da Caixa' : 'Código de Entrega';
        elements.codeInput.placeholder = mode === 'box' ? 'BOX-123-ABC' : 'BANANA-123';
        elements.validateBtn.innerHTML = mode === 'box' ? '📦 Confirmar Caixa' : '✅ Confirmar Entrega';
        elements.validateBtn.className = 'btn ' + (mode === 'box' ? 'btn-primary' : 'btn-success');
    ...
    async function validateCode() {
        ...
        if (!code) {
            showError('Digite ou escaneie um código');
            return;
        }
        ...
        try {
            const action = currentMode === 'box' ? 'validate_box_qr' : 'validate_delivery_qr';
            
            const formData = new FormData();
            formData.append('code', code);
            // Usar session ou token temporário ao invés de expor ID diretamente
            formData.append('delivery_token', <?= json_encode($_SESSION['delivery_token'] ?? '', JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT) ?>);
            if (orderId) formData.append('order_id', orderId);
            if (routeId) formData.append('route_id', routeId);
            
            // Adicionar token CSRF no cabeçalho PHP
            formData.append('csrf_token', '<?= $_SESSION['csrf_token'] ?? '' ?>');
            
            const response = await fetch('/mercado/api/qrcode.php?action=' + action, {
                method: 'POST',
                body: formData,
                headers: {
            ...
            if (data.success) {
                ...
                setTimeout(() => document.getElementById('codeInput').classList.remove('error'), 300);
            }
        } catch (e) {
            hideLoading();
    ...
    function showLoading() {
        ...
        document.querySelector('.manual-section').style.display = 'none';
    }
    ...
    function showSuccess(data) {
        ...
        if (currentMode === 'box') {
            title.textContent = 'Caixa Confirmada!';
            message.textContent = 'Você pegou a caixa certa. Bora entregar!';
            
            if (data.order) {
                details.innerHTML = `
                    <div class="item">
                        <span class="label">Pedido</span>
                        <span class="value">#${data.order.order_id}</span>
                    </div>
                    <div class="item">
                    ...
                    <div class="item">
                        ...
                        <span class="value">${data.order.address}</span>
                    </div>
                    <div class="item">
                        <span class="label">Código Entrega</span>
                        <span class="value" style="color: var(--brand); font-family: monospace;">${data.order.delivery_code}</span>
                    </div>
                `;
                orderId = data.order.order_id;
            }
            
            btn.textContent = 'Iniciar Entrega';
            btn.onclick = () => window.location.href = 'entrega.php?order_id=' + orderId;
        } else {
            title.textContent = 'Entrega Confirmada!';
            message.textContent = data.message || 'Entrega realizada com sucesso!';
            details.innerHTML = '';
            btn.textContent = 'Próxima Entrega';
            btn.onclick = () => window.location.href = 'index.php';
        }
        
        // Esconder scanner e input
        document.getElementById('scannerSection').style.display = 'none';
        document.querySelector('.manual-section').style.display = 'none';
    ...
    function handleResultAction() {
        // Definido dinamicamente em showSuccess
    }
