<?php
/**
 * CHAT DELIVERY ↔ SHOPPER - OneMundo Mercado
 */
session_start();
...
// Buscar pedido
$stmt = $pdo->prepare("SELECT o.*, s.name as shopper_name FROM om_market_orders o LEFT JOIN om_market_shoppers s ON o.shopper_id = s.id WHERE o.order_id = ? AND o.delivery_id = ?");
...
<head>
    ...
    <title>Chat - Pedido #<?= $order_id ?></title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .header { background: #1e293b; padding: 15px 20px; display: flex; align-items: center; gap: 15px; border-bottom: 1px solid #334155; }
        .back { color: #38bdf8; text-decoration: none; font-size: 24px; }
        .header-info h2 { font-size: 16px; }
        .header-info p { font-size: 12px; color: #64748b; }
        .messages { flex: 1; overflow-y: auto; padding: 20px; display: flex; flex-direction: column; gap: 10px; }
        .message { max-width: 80%; padding: 12px 16px; border-radius: 16px; font-size: 14px; }
        .message.sent { align-self: flex-end; background: #22c55e; color: white; border-bottom-right-radius: 4px; }
        .message.received { align-self: flex-start; background: #334155; border-bottom-left-radius: 4px; }
        .message.system { align-self: center; background: #1e293b; color: #64748b; font-size: 12px; }
        ...
        .input-area input { flex: 1; padding: 12px 16px; background: #0f172a; border: 1px solid #334155; border-radius: 25px; color: #e2e8f0; font-size: 14px; }
        .input-area button { width: 45px; height: 45px; background: #22c55e; border: none; border-radius: 50%; color: white; font-size: 18px; cursor: pointer; }
        .empty { text-align: center; color: #64748b; padding: 50px; }
    </style>
...
<body>
    <div class="header">
        <a href="entrega.php?id=<?= $order_id ?>" class="back">←</a>
        <div class="header-info">
            <h2>👩‍🦰 <?= htmlspecialchars($order["shopper_name"] ?? "Shopper") ?></h2>
            <p>Pedido #<?= $order_id ?></p>
        </div>
    </div>
    
    <div class="messages" id="messages">
        <div class="empty">💬 Carregando mensagens...</div>
    </div>
    
    <div class="input-area">
        <input type="text" id="msgInput" placeholder="Digite uma mensagem..." onkeyup="if(event.key==='Enter')sendMessage()">
    ...
    async function loadMessages() {
        try {
            const res = await fetch(`/mercado/api/chat_delivery.php?action=get&order_id=${ORDER_ID}&after_id=${lastMsgId}`);
            const data = await res.json();
            if (data.success && data.messages) {
                const container = document.getElementById("messages");
                if (data.messages.length > 0 && lastMsgId === 0) container.innerHTML = "";
                
                data.messages.forEach(m => {
                    const div = document.createElement("div");
                    let cls = "message ";
                    if (m.sender_type === "delivery") cls += "sent";
                    else if (m.sender_type === "system") cls += "system";
                    else cls += "received";
                    div.className = cls;
                    div.innerHTML = m.message + "<div class='time'>" + new Date(m.created_at).toLocaleTimeString("pt-BR", {hour:"2-digit",minute:"2-digit"}) + "</div>";
    ...
    async function sendMessage() {
        ...
        await fetch("/mercado/api/chat_delivery.php", {
            ...
            body: JSON.stringify({ action: "send", order_id: ORDER_ID, sender_type: "delivery", sender_id: DELIVERY_ID, message: msg })
        });
        loadMessages();
    }
