...
$mes = date("Y-m");
$stmt = $pdo->prepare("SELECT COUNT(*) as pedidos, COALESCE(SUM(total), 0) as total FROM oc_order WHERE customer_id = ? AND order_status_id > 0 AND DATE_FORMAT(date_added, '%Y-%m') = ?");
$stmt->execute([$customer_id, $mes]);
$este_mes = $stmt->fetch();
$stmt->execute([$customer_id, date("Y-m", strtotime("-1 month"))]);
$mes_passado = $stmt->fetch();
$meses = [];
for ($i = 5; $i >= 0; $i--) {
    $m = date("Y-m", strtotime("-$i months"));
    $stmt->execute([$customer_id, $m]);
    $d = $stmt->fetch();
    $meses[] = ["mes" => date("M", strtotime($m."-01")), "total" => $d["total"]];
}
$max = max(array_column($meses, "total")) ?: 1;
$stmt = $pdo->prepare("SELECT * FROM om_budget_goals WHERE customer_id = ? AND month_year = ?");
$stmt->execute([$customer_id, $mes]);
$meta = $stmt->fetch();
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["budget"])) {
    $budget = (float)$_POST["budget"];
    if ($budget > 0) {
        $pdo->prepare("INSERT INTO om_budget_goals (customer_id, month_year, budget_limit, spent) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE budget_limit = ?")->execute([$customer_id, $mes, $budget, $este_mes["total"], $budget]);
...
<title>Meus Gastos - Mercado OneMundo</title>
...
<body>
<header class="header"><div class="header-inner"><a href="/mercado/perfil/" class="back-btn">← Voltar</a><h1 class="page-title">📊 Meus Gastos</h1></div></header>
<main class="main">
<div class="card">
    <h3 class="card-title">📅 Este Mês</h3>
    <div style="font-size:2.5rem;font-weight:900">R$ <?= number_format($este_mes["total"], 2, ",", ".") ?></div>
    <?php $diff = $este_mes["total"] - $mes_passado["total"]; $pct = $mes_passado["total"] > 0 ? abs($diff / $mes_passado["total"]) * 100 : 0; ?>
    <div style="font-size:14px;margin-top:8px;color:<?= $diff >= 0 ? "#ef4444" : "var(--primary)" ?>"><?= $diff >= 0 ? "↑" : "↓" ?> <?= number_format($pct, 0) ?>% vs mês passado</div>
    <div style="font-size:14px;color:var(--gray-500);margin-top:4px"><?= $este_mes["pedidos"] ?> pedidos</div>
</div>
<div class="card">
    ...
    <div style="display:flex;align-items:flex-end;gap:8px;height:120px;margin-top:16px">
        ...
        <div style="flex:1;background:var(--primary);border-radius:8px 8px 0 0;height:<?= ($m["total"] / $max) * 100 ?>%;min-height:8px;position:relative"><span style="position:absolute;bottom:-20px;left:50%;transform:translateX(-50%);font-size:10px;color:var(--gray-500)"><?= $m["mes"] ?></span></div>
        <?php endforeach; ?>
    </div>
</div>
<div class="card">
    <h3 class="card-title">🎯 Meta de Orçamento</h3>
    <?php if ($meta): $pct = min(100, ($este_mes["total"] / $meta["budget_limit"]) * 100); ?>
    <div style="background:var(--gray-200);height:24px;border-radius:12px;overflow:hidden;margin:16px 0"><div style="background:<?= $pct < 80 ? "var(--primary)" : ($pct < 100 ? "#f59e0b" : "#ef4444") ?>;height:100%;width:<?= $pct ?>%"></div></div>
    <div style="display:flex;justify-content:space-between;font-size:14px;color:var(--gray-500)"><span>R$ <?= number_format($este_mes["total"], 0, ",", ".") ?></span><span>Meta: R$ <?= number_format($meta["budget_limit"], 0, ",", ".") ?></span></div>
    <?php if ($pct >= 100): ?><div style="color:#ef4444;font-weight:700;margin-top:12px">⚠️ Você ultrapassou a meta!</div><?php elseif ($pct >= 80): ?><div style="color:#f59e0b;font-weight:700;margin-top:12px">⚠️ Atenção: <?= number_format(100 - $pct, 0) ?>% restante</div><?php else: ?><div style="color:var(--primary);font-weight:700;margin-top:12px">✅ Dentro do orçamento</div><?php endif; ?>
    <?php else: ?>
    <p style="color:var(--gray-500);margin-bottom:16px">Defina uma meta mensal para controlar seus gastos</p>
    <form method="POST" style="display:flex;gap:8px">
        <input type="number" name="budget" placeholder="R$ 500" required style="flex:1;padding:12px;border:1px solid var(--gray-200);border-radius:10px;font-size:16px">
