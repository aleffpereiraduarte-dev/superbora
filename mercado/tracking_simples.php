...
// Buscar pedido com validação de acesso
...
// TODO: Adicionar validação de propriedade do pedido baseada em sessão/token
...
// Coordenadas - forçar valores se vazios
...
$delivery_lat = !empty($delivery['lat']) ? (float)$delivery['lat'] : -23.5415;
$delivery_lng = !empty($delivery['lng']) ? (float)$delivery['lng'] : -46.6333;
...
<head>
    ...
    <title>🗺️ Tracking #<?= $order_id ?></title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .status { display: inline-block; padding: 4px 12px; border-radius: 20px; font-size: 13px; font-weight: 600; margin-top: 8px; }
        .status.delivering { background: #f9731620; color: #f97316; }
        .status.other { background: #64748b20; color: #64748b; }
        #map { width: 100%; height: 400px; }
        .debug { padding: 16px; font-size: 12px; font-family: monospace; background: #1e293b; margin: 16px; border-radius: 8px; }
...
<div class="header">
    <h1>🗺️ Tracking Pedido #<?= htmlspecialchars($order['order_number'], ENT_QUOTES, 'UTF-8') ?></h1>
    <div class="info"><?= htmlspecialchars($order['delivery_name'] ?: 'Sem entregador', ENT_QUOTES, 'UTF-8') ?></div>
    <div class="status <?= $is_delivering ? 'delivering' : 'other' ?>">
        <?= $order['status'] ?>
    </div>
</div>
...
<div class="debug">
    <div class="<?= $is_delivering ? 'ok' : 'fail' ?>">is_delivering: <?= $is_delivering ? 'true' : 'false' ?></div>
    <div class="<?= $dest_lat ? 'ok' : 'fail' ?>">dest_lat: <?= $dest_lat ?></div>
    <div class="<?= $dest_lng ? 'ok' : 'fail' ?>">dest_lng: <?= $dest_lng ?></div>
    <div class="<?= $delivery_lat ? 'ok' : 'fail' ?>">delivery_lat: <?= $delivery_lat ?></div>
    <div class="<?= $delivery_lng ? 'ok' : 'fail' ?>">delivery_lng: <?= $delivery_lng ?></div>
</div>
<?php endif; ?>
...
console.log('Coordenadas:', { DEST_LAT, DEST_LNG, DELIVERY_LAT, DELIVERY_LNG });

// Criar mapa SEMPRE (para teste)
try {
    const map = L.map('map').setView([DELIVERY_LAT, DELIVERY_LNG], 14);
    ...
    L.marker([DELIVERY_LAT, DELIVERY_LNG], { icon: deliveryIcon }).addTo(map).bindPopup('🚴 Entregador');
    ...
    L.marker([DEST_LAT, DEST_LNG], { icon: destIcon }).addTo(map).bindPopup('🏠 Destino');
    
    // Linha entre eles
    L.polyline([[DELIVERY_LAT, DELIVERY_LNG], [DEST_LAT, DEST_LNG]], {
        color: '#f97316',
        weight: 3,
        opacity: 0.7,
        dashArray: '10, 10'
    ...
    // Ajustar zoom
    const bounds = L.latLngBounds([[DELIVERY_LAT, DELIVERY_LNG], [DEST_LAT, DEST_LNG]]);
    map.fitBounds(bounds, { padding: [50, 50] });
    
    console.log('✅ Mapa criado!');
} catch (e) {
    console.error('❌ Erro:', e);
    document.getElementById('map').innerHTML = '<div style="padding:20px;color:red;">Erro: ' + e.message + '</div>';
}
</script>
