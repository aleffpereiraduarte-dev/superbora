<?php
session_start();
// Em produção, logar erros sem exibi-los
...
// Definir valores padrão seguros ou falhar se não configurado
$db_host = '147.93.12.236';
$db_name = defined("DB_DATABASE") ? DB_DATABASE : null;
...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($_POST["action"] === "criar" && trim($_POST["nome"])) {
        ...
        // Validar ícone (apenas emojis simples)
        if (strlen($icon) > 10) {
            $icon = "📝";
        }
        
        $pdo->prepare("INSERT INTO om_shopping_lists (customer_id, name, icon) VALUES (?, ?, ?)")->execute([$customer_id, $nome, $icon]);
    }
    
    if ($_POST["action"] === "excluir" && is_numeric($_POST["list_id"])) {
        $list_id = (int)$_POST["list_id"];
        $pdo->prepare("DELETE FROM om_shopping_list_items WHERE list_id = ?")->execute([$list_id]);
        $pdo->prepare("DELETE FROM om_shopping_lists WHERE list_id = ? AND customer_id = ?")->execute([$list_id, $customer_id]);
    }
    
    header("Location: listas.php"); exit;
}
$stmt = $pdo->prepare("
    SELECT l.*, COALESCE(i.items, 0) as items 
    FROM om_shopping_lists l 
    LEFT JOIN (
        SELECT list_id, COUNT(*) as items 
        FROM om_shopping_list_items 
        GROUP BY list_id
    ) i ON l.list_id = i.list_id 
    WHERE l.customer_id = ? 
...
<title>Minhas Listas - Mercado OneMundo</title>
...
<body>
<header class="header"><div class="header-inner"><a href="/mercado/perfil/" class="back-btn">← Voltar</a><h1 class="page-title">📝 Minhas Listas</h1></div></header>
<main class="main">
/* Adicionar ao CSS */
.new-list-card {
...
<div class="new-list-card">
    <h3 style="margin-bottom:12px">➕ Nova Lista</h3>
    <?php
// Gerar token CSRF no início do arquivo
...
<form method="POST" style="display:flex;gap:8px">
        ...
        <input type="text" name="nome" placeholder="Nome da lista..." required style="flex:1;padding:12px;border:none;border-radius:10px;font-size:14px">
        <button type="submit" style="padding:12px 20px;background:white;color:var(--primary);border:none;border-radius:10px;font-weight:700;cursor:pointer">Criar</button>
    </form>
</div>
<?php if (empty($listas)): ?>
<div class="empty"><div class="empty-icon">📝</div><p>Nenhuma lista criada</p></div>
<?php else: foreach ($listas as $l): ?>
<div class="card" style="display:flex;align-items:center;gap:16px;cursor:pointer;margin-bottom:12px" onclick="location.href='lista.php?id=<?= (int)$l["list_id"] ?>'">
    <span style="font-size:32px"><?= $l["icon"] ?></span>
    <div style="flex:1"><div style="font-weight:700"><?= htmlspecialchars($l["name"]) ?></div><div style="font-size:13px;color:var(--gray-500)"><?= $l["items"] ?> itens</div></div>
