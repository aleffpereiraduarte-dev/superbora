...
$pdo = new PDO("mysql:host=".(defined("DB_HOSTNAME")?DB_HOSTNAME:"localhost").";dbname=".(defined("DB_DATABASE")?DB_DATABASE:"mercado").";charset=utf8mb4", defined("DB_USERNAME")?DB_USERNAME:getenv('DB_USERNAME'), defined("DB_PASSWORD")?DB_PASSWORD:getenv('DB_PASSWORD'), [PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION, PDO::ATTR_DEFAULT_FETCH_MODE=>PDO::FETCH_ASSOC]);
...
if (isset($_GET["remove"])) {
    ...
    if ($product_id && $product_id > 0) {
        $pdo->prepare("DELETE FROM om_customer_favorites WHERE customer_id = ? AND product_id = ?")->execute([$customer_id, $product_id]);
    }
    header("Location: favoritos.php"); exit;
}
$stmt = $pdo->prepare("SELECT f.*, p.image, pd.name, p.price, ps.price as special FROM om_customer_favorites f JOIN oc_product p ON f.product_id = p.product_id JOIN oc_product_description pd ON p.product_id = pd.product_id AND pd.language_id = 2 LEFT JOIN oc_product_special ps ON p.product_id = ps.product_id AND (ps.date_end IS NULL OR ps.date_end > NOW()) WHERE f.customer_id = ? ORDER BY f.created_at DESC");
...
<title>Favoritos - Mercado OneMundo</title>
...
<body>
<header class="header"><div class="header-inner"><a href="/mercado/perfil/" class="back-btn">← Voltar</a><h1 class="page-title">❤️ Favoritos</h1></div></header>
<main class="main">
<?php if (empty($favoritos)): ?>
<div class="empty"><div class="empty-icon">❤️</div><p>Nenhum favorito ainda</p><a href="/mercado/" class="btn btn-primary" style="margin-top:20px">🛒 Explorar Produtos</a></div>
<?php else: ?>
<div class="products-grid">
<?php foreach ($favoritos as $f): $preco = $f["special"] ?: $f["price"]; ?>
<div class="card" style="margin:0;padding:16px">
    <div style="height:120px;background:var(--gray-100);border-radius:12px;display:flex;align-items:center;justify-content:center;margin-bottom:12px;position:relative">
        <?php if ($f["image"]): ?><img src="/image/<?= htmlspecialchars($f["image"]) ?>" style="max-width:90%;max-height:90%;object-fit:contain" onerror="this.style.display='none';this.nextElementSibling.style.display='block';"><div style="display:none;font-size:2em;">📦</div><?php else: ?><div style="font-size:2em;">📦</div><?php endif; ?>
        <button onclick="location.href='?remove=<?= (int)$f["product_id"] ?>'" style="position:absolute;top:8px;right:8px;width:28px;height:28px;border-radius:50%;background:white;border:none;cursor:pointer;font-size:14px">×</button>
    </div>
    <div style="font-size:13px;font-weight:600;margin-bottom:8px;height:36px;overflow:hidden"><?= htmlspecialchars($f["name"]) ?></div>
    <div style="font-size:18px;font-weight:800;color:var(--primary);margin-bottom:12px">R$ <?= number_format($preco, 2, ",", ".") ?></div>
    <button class="btn btn-primary" style="width:100%;justify-content:center;padding:10px" onclick="addCart(<?= (int)$f["product_id"] ?>)">+ Carrinho</button>
</div>
<?php endforeach; ?>
</div>
<?php endif; ?>
...
function addCart(id) {
    ...
    .then(data => {
        if(data.success) alert('✅ Adicionado!');
