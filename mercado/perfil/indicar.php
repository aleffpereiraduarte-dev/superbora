<?php
session_start();
// Remove error_reporting(0) em desenvolvimento
// Em produção, configure no php.ini: display_errors = Off, log_errors = On
$_oc = dirname(dirname(__DIR__));
if (file_exists($_oc."/config.php")) require_once($_oc."/config.php");
$pdo = new PDO("mysql:host=".(defined("DB_HOSTNAME")?DB_HOSTNAME:"localhost").";dbname=".(defined("DB_DATABASE")?DB_DATABASE:"app_db").";charset=utf8mb4", defined("DB_USERNAME")?DB_USERNAME:"app_user", defined("DB_PASSWORD")?DB_PASSWORD:getenv('DB_PASSWORD'), [PDO::ATTR_ERRMODE=>PDO::ERRMODE_EXCEPTION, PDO::ATTR_DEFAULT_FETCH_MODE=>PDO::FETCH_ASSOC]);
...
$stmt = $pdo->prepare("SELECT referrer_id, referral_code FROM om_referrals WHERE referrer_id = ? AND referred_id IS NULL LIMIT 1");
$stmt->execute([$customer_id]);
$ref = $stmt->fetch();
if (!$ref) {
    $code = strtoupper(substr(bin2hex(random_bytes(4)), 0, 8));
    $pdo->prepare("INSERT INTO om_referrals (referrer_id, referral_code) VALUES (?, ?)")->execute([$customer_id, $code]);
    $ref = ['referrer_id' => $customer_id, 'referral_code' => $code];
}
...
<div class="card" style="text-align:center">
    ...
    <div style="font-size:32px;font-weight:900;letter-spacing:4px;font-family:monospace;margin-bottom:16px"><?= htmlspecialchars($ref["referral_code"], ENT_QUOTES, 'UTF-8') ?></div>
    <div style="display:flex;gap:12px;justify-content:center">
        <button class="btn btn-primary" onclick="navigator.clipboard.writeText('<?= htmlspecialchars($ref["referral_code"], ENT_QUOTES, 'UTF-8') ?>');alert('✅ Copiado!')">📋 Copiar</button>
        <button class="btn btn-outline" onclick="navigator.share?.({text:'Use meu código <?= htmlspecialchars($ref["referral_code"], ENT_QUOTES, 'UTF-8') ?> no Mercado OneMundo e ganhe R$10!'})">📤 Compartilhar</button>
    </div>
</div>
