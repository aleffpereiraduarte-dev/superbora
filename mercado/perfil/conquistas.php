...
$customer_id = (int)($_SESSION["mercado_customer_id"] ?? $_SESSION["customer_id"] ?? 0);
...
$stmt = $pdo->prepare("SELECT * FROM om_badges WHERE status = ? ORDER BY category, requirement_value");
$stmt->execute([1]);
$badges = $stmt->fetchAll();
$stmt = $pdo->prepare("SELECT b.*, cb.progress, cb.completed_at FROM om_badges b LEFT JOIN om_customer_badges cb ON b.badge_id = cb.badge_id AND cb.customer_id = ? WHERE b.status = '1' ORDER BY b.category, b.requirement_value");
$stmt->execute([$customer_id]);
$badges = $stmt->fetchAll();
$my_badges = [];
$total = count($badges);
$done = count(array_filter($my_badges, fn($b) => $b["completed_at"]));
?><?php
...
<title>Conquistas - Mercado OneMundo</title>
...
<body>
<header class="header"><div class="header-inner"><a href="/mercado/perfil/" class="back-btn">← Voltar</a><h1 class="page-title">🏆 Conquistas</h1></div></header>
<main class="main">
<div class="stats-grid">
    <div class="card" style="text-align:center;margin:0"><div style="font-size:2rem;font-weight:900;color:var(--primary)"><?= $done ?></div><div style="font-size:13px;color:var(--gray-500)">Conquistados</div></div>
    <div class="card" style="text-align:center;margin:0"><div style="font-size:2rem;font-weight:900"><?= $total ?></div><div style="font-size:13px;color:var(--gray-500)">Total</div></div>
    <div class="card" style="text-align:center;margin:0"><div style="font-size:2rem;font-weight:900;color:var(--gray-400)"><?= $total - $done ?></div><div style="font-size:13px;color:var(--gray-500)">Restantes</div></div>
</div>
<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(140px,1fr));gap:16px">
<?php foreach ($badges as $b): $cb = $my_badges[$b["badge_id"]] ?? null; $done = $cb && $cb["completed_at"]; $pct = $cb ? min(100, ($cb["progress"] / max(1,$b["requirement_value"])) * 100) : 0; ?>
<div class="card" style="text-align:center;margin:0;opacity:<?= $done ? 1 : ($cb ? 0.8 : 0.5) ?>;border:<?= $done ? "2px solid var(--primary)" : "none" ?>;position:relative">
    <?php if ($done): ?><div style="position:absolute;top:8px;right:8px">✅</div><?php elseif (!$cb): ?><div style="position:absolute;top:8px;right:8px">🔒</div><?php endif; ?>
    <div style="font-size:40px;margin-bottom:8px" role="img" aria-label="<?= htmlspecialchars($b["name"], ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($b["icon"], ENT_QUOTES, 'UTF-8') ?></div>
    <div style="font-size:13px;font-weight:700"><?= htmlspecialchars($b["name"]) ?></div>
    <div style="font-size:11px;color:var(--gray-500);margin:4px 0"><?= htmlspecialchars($b["description"]) ?></div>
    <?php if ($b["cashback_reward"] > 0): ?><div style="font-size:12px;font-weight:700;color:var(--primary)">R$<?= $b["cashback_reward"] ?></div><?php endif; ?>
    <?php if (!$done && $cb): ?>
    <div style="margin-top:8px;background:var(--gray-200);height:6px;border-radius:3px;overflow:hidden"><div style="background:var(--primary);height:100%;width:<?= $pct ?>%"></div></div>
    <div style="font-size:10px;color:var(--gray-500);margin-top:4px"><?= $cb["progress"] ?>/<?= $b["requirement_value"] ?></div>
    <?php endif; ?>
</div>
<?php endforeach; ?>
</div>
