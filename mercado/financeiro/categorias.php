...
<body>
    ...
    <main class="main">
        ...
        <div class="card">
            <h3 class="card-title"><i class="fas fa-arrow-down"></i> Categorias de Despesas</h3>
            <table class="table">
                <thead>
                    <tr>
                        <th>Código</th>
                ...
                <tbody id="tableExpenses">
                    <tr><td colspan="3"><i class="fas fa-spinner fa-spin"></i></td></tr>
                </tbody>
            </table>
        </div>
        
        <div class="card">
            <h3 class="card-title"><i class="fas fa-arrow-up"></i> Categorias de Receitas</h3>
            <table class="table">
                <thead>
                    <tr>
                        <th>Código</th>
        ...
        <script>
            async function loadCategories() {
                // Despesas
                let r = await fetch("api/categories.php?action=list&type=expense");
                let d = await r.json();
                document.getElementById("tableExpenses").innerHTML = (d.success && d.categories && d.categories.length) 
                    ? d.categories.map(c => `<tr><td>${c.code || "-"}</td><td>${c.name}</td><td><span class="badge badge-success">Ativo</span></td></tr>`).join("") 
                    : `<tr><td colspan="3">Nenhuma categoria</td></tr>`;
                
                // Receitas
                r = await fetch("api/categories.php?action=list&type=income");
                d = await r.json();
                document.getElementById("tableIncome").innerHTML = (d.success && d.categories && d.categories.length) 
                    ? d.categories.map(c => `<tr><td>${c.code || "-"}</td><td>${c.name}</td><td><span class="badge badge-success">Ativo</span></td></tr>`).join("") 
                    : `<tr><td colspan="3">Nenhuma categoria</td></tr>`;
            }
            loadCategories();
