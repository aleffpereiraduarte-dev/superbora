...
<body>
    ...
    <main class="main">
        <?php $pageTitle = "Centros de Custo"; ?>
        <header class="header">
            <h1><i class="fas fa-sitemap"></i> Centros de Custo</h1>
        </header>
        ...
        <script>
            async function loadCostCenters() {
                ...
                if (d.success && d.cost_centers && d.cost_centers.length > 0) {
                    tbody.innerHTML = d.cost_centers.map(c => `<tr>
                        <td><strong>${c.code}</strong></td>
                        <td>${c.name}</td>
                        <td><span class="badge badge-info">${c.type || "Geral"}</span></td>
                        <td><span class="badge badge-success">Ativo</span></td>
                    </tr>`).join("");
                } else {
                    tbody.innerHTML = `<tr><td colspan="4" class="empty-state"><i class="fas fa-sitemap"></i><br>Nenhum centro de custo</td></tr>`;
                }
            }
