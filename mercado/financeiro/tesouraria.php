...
<body>
    ...
    <main class="main">
        <?php $pageTitle = "Tesouraria"; ?>
        <header class="header">
            <h1><i class="fas fa-university"></i> Tesouraria</h1>
            <div class="header-right">
                <button class="btn btn-primary" onclick="openModalConta()"><i class="fas fa-plus"></i> Nova Conta</button>
                <button class="btn btn-secondary" onclick="openModalTransfer()"><i class="fas fa-exchange-alt"></i> Transferência</button>
            </div>
        </header>
        
        <div class="stats-row">
            <div class="stat-card">
                <div class="stat-value" style="color:var(--primary)" id="saldoTotal">R$ 0,00</div>
                <div class="stat-label">Saldo Total</div>
            </div>
        </div>
        
        <div class="card">
            <h3 class="card-title"><i class="fas fa-piggy-bank"></i> Contas Bancárias</h3>
            <table class="table">
                <thead>
                    <tr>
                        <th>Conta</th>
        ...
        <script>
            async function loadAccounts() {
                const r = await fetch("api/bank.php?action=getSummary");
                const d = await r.json();
                
                if (d.success) {
                    document.getElementById("saldoTotal").textContent = formatMoney(d.total_balance);
                    
                    const tbody = document.getElementById("tableContas");
                    if (d.accounts && d.accounts.length > 0) {
                        tbody.innerHTML = d.accounts.map(a => `<tr>
                            <td><strong>${a.account_name}</strong></td>
                            <td>${a.bank_name || "-"}</td>
                            <td>${a.agency || "-"} / ${a.account_number || "-"}</td>
                            <td><strong style="color:${a.current_balance >= 0 ? "var(--primary)" : "var(--danger)"}">${formatMoney(a.current_balance)}</strong></td>
                            <td class="actions">
                                <button class="btn-view" title="Ver extrato"><i class="fas fa-list"></i></button>
                            </td>
                        </tr>`).join("");
                    } else {
                        tbody.innerHTML = `<tr><td colspan="5" class="empty-state"><i class="fas fa-university"></i><br>Nenhuma conta cadastrada</td></tr>`;
                    }
                }
            }
            
            function openModalConta() { alert("Em desenvolvimento"); }
            function openModalTransfer() { alert("Em desenvolvimento"); }
