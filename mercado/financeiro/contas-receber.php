...
<body>
    ...
    <main class="main">
        <?php $pageTitle = "Contas a Receber"; ?>
        <header class="header">
            <h1><i class="fas fa-hand-holding-usd"></i> Contas a Receber</h1>
            <div class="header-right">
                <?php if($perms["create"]): ?>
                <button class="btn btn-primary" onclick="openModal()"><i class="fas fa-plus"></i> Nova Receita</button>
                <?php endif; ?>
            </div>
        </header>
        ...
        <div class="stats-row">
            ...
            <div class="stat-card">
                <div class="stat-value" style="color:var(--info)" id="statPrevisto">R$ 0</div>
                <div class="stat-label">Valor Previsto</div>
            </div>
            <div class="stat-card">
                <div class="stat-value" style="color:var(--primary)" id="statRecebido">R$ 0</div>
        ...
        <script>
            async function loadStats() {
                const r = await fetch("api/receivables.php?action=getStats");
                const d = await r.json();
                if (d.success) {
                    document.getElementById("statTotal").textContent = d.stats.total;
                    document.getElementById("statPrevisto").textContent = formatMoney(d.stats.valor_previsto);
                    document.getElementById("statRecebido").textContent = formatMoney(d.stats.recebido_mes);
                }
            }
            
            async function loadReceivables() {
                const r = await fetch("api/receivables.php?action=list");
                const d = await r.json();
                const tbody = document.getElementById("tableBody");
                
                if (d.success && d.receivables && d.receivables.length > 0) {
                    tbody.innerHTML = d.receivables.map(p => {
                        let badge = "badge-info";
                        if (p.status === "recebido") badge = "badge-success";
                        else if (p.status === "previsto") badge = "badge-warning";
                        
                        return `<tr>
                            <td><strong>${p.customer_name || "N/I"}</strong></td>
                            <td>${p.description || "-"}</td>
                            <td>${formatDate(p.due_date)}</td>
                            <td><strong>${formatMoney(p.net_value)}</strong></td>
                            <td><span class="badge ${badge}">${p.status}</span></td>
                            <td class="actions">
                                <button class="btn-pay" title="Receber"><i class="fas fa-dollar-sign"></i></button>
                            </td>
                        </tr>`;
                    }).join("");
                } else {
                    tbody.innerHTML = `<tr><td colspan="6" class="empty-state"><i class="fas fa-inbox"></i><br>Nenhuma conta a receber</td></tr>`;
                }
            }
