...
$requests = apcu_fetch($rate_limit_key) ?: 0;
...
apcu_store($rate_limit_key, $requests + 1, 3600); // 1 hora
...
function geocodificar($endereco) {
    ...
    return !empty($gd[0]['lat']) ? $gd[0] : null;
}

function getCoords($cep) {
    $cep = preg_replace('/\D/', '', $cep);
    $dadosCep = buscarCEP($cep);
    if (!$dadosCep) return null;
...
if (!preg_match('/^\d{8}$/', $cep)) {
    echo json_encode(['success' => false, 'error' => 'CEP deve conter exatamente 8 dígitos', 'disponivel' => false]);
    exit;
}
