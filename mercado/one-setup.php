...
// Usar prepared statements reutilizáveis e consultas otimizadas
$queries = [
    'cards' => "SELECT * FROM om_one_saved_cards WHERE customer_id = ? AND is_active = 1 ORDER BY is_default DESC",
    'addresses' => "SELECT * FROM om_one_saved_addresses WHERE customer_id = ? AND is_active = 1 ORDER BY is_default DESC",
    'settings' => "SELECT * FROM om_one_quick_buy_settings WHERE customer_id = ?"
];
...
foreach ($queries as $key => $query) {
    ...
    $results[$key] = ($key === 'settings') ? $stmt->fetch(PDO::FETCH_ASSOC) : $stmt->fetchAll(PDO::FETCH_ASSOC);
}

[$cards, $addresses, $settings] = [$results['cards'], $results['addresses'], $results['settings']];
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <meta http-equiv="Content-Security-Policy" content="default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline' fonts.googleapis.com; font-src fonts.gstatic.com; connect-src 'self' viacep.com.br;">
    <title>⚡ Configurar Compra Rápida - ONE</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="/assets/css/one-setup.css">
    <style>
        :root { --primary: #22c55e; --bg: #0a0a0a; --card: #141414; --border: #262626; --text: #fff; --muted: #888; }
        ...
        .back-btn { background: var(--card); border: 1px solid var(--border); color: var(--text); width: 40px; height: 40px; border-radius: 12px; font-size: 18px; cursor: pointer; }
        .header h1 { font-size: 20px; }
        
        .section { background: var(--card); border: 1px solid var(--border); border-radius: 16px; padding: 20px; margin-bottom: 16px; }
        .section-title { font-size: 14px; color: var(--muted); margin-bottom: 16px; text-transform: uppercase; letter-spacing: 0.5px; }
        
        .card-option, .address-option { display: flex; align-items: center; gap: 12px; padding: 16px; background: rgba(255,255,255,0.03); border-radius: 12px; margin-bottom: 10px; cursor: pointer; border: 2px solid transparent; transition: all 0.2s; }
        .card-option:hover, .address-option:hover { border-color: var(--primary); }
        .card-option.selected, .address-option.selected { border-color: var(--primary); background: rgba(34,197,94,0.1); }
        
        .card-icon { font-size: 24px; }
        .card-info { flex: 1; }
        .card-number { font-weight: 600; font-size: 15px; }
        .card-holder { font-size: 13px; color: var(--muted); }
        
        .address-info { flex: 1; }
        .address-street { font-weight: 600; font-size: 15px; }
        .address-detail { font-size: 13px; color: var(--muted); }
        
        .check-circle { width: 24px; height: 24px; border: 2px solid var(--border); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: transparent; }
        .selected .check-circle { border-color: var(--primary); background: var(--primary); color: #000; }
        
        .add-new { display: flex; align-items: center; justify-content: center; gap: 8px; padding: 16px; border: 2px dashed var(--border); border-radius: 12px; color: var(--muted); cursor: pointer; transition: all 0.2s; }
        .add-new:hover { border-color: var(--primary); color: var(--primary); }
        ...
        .toggle::after { content: ""; position: absolute; width: 22px; height: 22px; background: var(--text); border-radius: 50%; top: 3px; left: 3px; transition: transform 0.2s; }
        .toggle.active::after { transform: translateX(22px); }
        
        .input-group { margin-top: 12px; }
        .input-group label { font-size: 13px; color: var(--muted); display: block; margin-bottom: 6px; }
        .input-group input { width: 100%; background: var(--bg); border: 1px solid var(--border); border-radius: 10px; padding: 12px 16px; color: var(--text); font-size: 15px; }
        
        .save-btn { position: fixed; bottom: 20px; left: 20px; right: 20px; background: linear-gradient(135deg, var(--primary) 0%, #16a34a 100%); color: #000; border: none; padding: 16px; border-radius: 14px; font-size: 16px; font-weight: 700; cursor: pointer; }
        
        /* Modal */
        .modal { position: fixed; inset: 0; background: rgba(0,0,0,0.8); display: none; align-items: flex-end; justify-content: center; z-index: 1000; }
        .modal.show { display: flex; }
        .modal-content { background: var(--card); border-top-left-radius: 24px; border-top-right-radius: 24px; width: 100%; max-height: 90vh; overflow-y: auto; padding: 24px; animation: slideUp 0.3s ease; }
        @keyframes slideUp { from { transform: translateY(100%); } to { transform: translateY(0); } }
        .modal-header { display: flex; align-items: center; justify-content: space-between; margin-bottom: 24px; }
        .modal-title { font-size: 18px; font-weight: 700; }
        .modal-close { background: var(--border); border: none; color: var(--text); width: 36px; height: 36px; border-radius: 50%; font-size: 20px; cursor: pointer; }
        
        .form-row { margin-bottom: 16px; }
        .form-row label { font-size: 13px; color: var(--muted); display: block; margin-bottom: 6px; }
        .form-row input { width: 100%; background: var(--bg); border: 1px solid var(--border); border-radius: 10px; padding: 14px 16px; color: var(--text); font-size: 16px; }
        .form-row-half { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
        
        .modal-btn { width: 100%; background: var(--primary); color: #000; border: none; padding: 16px; border-radius: 12px; font-size: 16px; font-weight: 700; cursor: pointer; margin-top: 12px; }
    </style>
...
<body>
    ...
    <!-- Cartões -->
    <div class="section">
        <div class="section-title">💳 Cartão de Pagamento</div>
        
        <?php foreach ($cards as $card): ?>
        <div class="card-option <?php echo ($settings && $settings["default_card_id"] == $card["card_id"]) ? "selected" : ""; ?>" data-card-id="<?php echo $card["card_id"]; ?>" onclick="selectCard(this, <?php echo $card["card_id"]; ?>)">
            <div class="card-icon"><?php echo $card["card_brand"] == "visa" ? "💳" : "💳"; ?></div>
            <div class="card-info">
                <div class="card-number"><?php echo htmlspecialchars(strtoupper($card["card_brand"]), ENT_QUOTES, 'UTF-8'); ?> ••••<?php echo htmlspecialchars($card["card_last4"], ENT_QUOTES, 'UTF-8'); ?></div>
        ...
        <div class="add-new" onclick="showModal(\"cardModal\")">
    ...
    <div class="section">
        ...
        <div class="address-option <?php echo ($settings && $settings["default_address_id"] == $addr["address_id"]) ? "selected" : ""; ?>" data-address-id="<?php echo $addr["address_id"]; ?>" onclick="selectAddress(this, <?php echo $addr["address_id"]; ?>)">
            <div class="card-icon">🏠</div>
            <div class="address-info">
                <div class="address-street"><?php echo htmlspecialchars($addr["street"] . ", " . $addr["number"]); ?></div>
                <div class="address-detail"><?php echo htmlspecialchars($addr["neighborhood"] . " - " . $addr["city"]); ?></div>
            </div>
            <div class="check-circle">✓</div>
        </div>
        <?php endforeach; ?>
        
        <div class="add-new" onclick="showModal(\"addressModal\")">
    ...
    <div class="section">
        ...
        <div class="setting-row">
            <div>
                <div class="setting-label">Usar créditos automaticamente</div>
                <div class="setting-desc">Aplica seus créditos em todas as compras</div>
            </div>
            <div class="toggle <?php echo (!$settings || $settings["auto_apply_credits"]) ? "active" : ""; ?>" id="autoCredits" onclick="toggleSetting(this)"></div>
        </div>
        
        <div class="setting-row">
            <div>
                <div class="setting-label">Pedir PIN para compras</div>
                <div class="setting-desc">Segurança extra para valores altos</div>
            </div>
            <div class="toggle <?php echo ($settings && $settings["require_pin"]) ? "active" : ""; ?>" id="requirePin" onclick="toggleSetting(this)"></div>
        </div>
        
        <div class="input-group">
            <label>Limite diário (R$)</label>
            <input type="number" id="dailyLimit" value="<?php echo $settings ? $settings["daily_limit"] : 500; ?>" placeholder="500.00">
        </div>
    </div>
    ...
    <div class="modal" id="cardModal">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button class="modal-close" onclick="hideModal(\"cardModal\")">×</button>
            </div>
            ...
            <button class="modal-btn" onclick="saveCard()">Salvar Cartão</button>
        </div>
    </div>
    ...
    <div class="modal" id="addressModal">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button class="modal-close" onclick="hideModal(\"addressModal\")">×</button>
            </div>
            
            <div class="form-row">
                <label>CEP</label>
                <input type="text" id="addrZip" placeholder="00000-000" maxlength="9" onblur="searchCep()">
            </div>
            <div class="form-row">
                <label>Rua</label>
                <input type="text" id="addrStreet" placeholder="Nome da rua">
            </div>
            <div class="form-row-half">
                <div class="form-row">
                    <label>Número</label>
                    <input type="text" id="addrNumber" placeholder="123">
                </div>
                <div class="form-row">
                    <label>Complemento</label>
                    <input type="text" id="addrComplement" placeholder="Apto 101">
                </div>
            </div>
            <div class="form-row">
                <label>Bairro</label>
            ...
            <button class="modal-btn" onclick="saveAddress()">Salvar Endereço</button>
        </div>
    </div>
    
    <script>
        let selectedCardId = <?php echo $settings && $settings["default_card_id"] ? $settings["default_card_id"] : "null"; ?>;
        let selectedAddressId = <?php echo $settings && $settings["default_address_id"] ? $settings["default_address_id"] : "null"; ?>;
        ...
        function formatCardNumber(input) {
            ...
            const isValid = value.replace(/\s/g, '').length >= 13 && value.replace(/\s/g, '').length <= 19;
            input.setCustomValidity(isValid ? '' : 'Número de cartão inválido');
        }
...
const data = {
            ...
            const res = await fetch("/mercado/api/one.php", {
                method: "POST",
                headers: { 
        ...
        async function searchCep() {
            const cep = document.getElementById("addrZip").value.replace(/\D/g, "");
            ...
            if (!res.ok) throw new Error('CEP service unavailable');
        ...
        async function saveSettings() {
            const data = {
                action: "setup_quick_buy",
                customer_id: <?php echo $customer_id; ?>,
                card_id: selectedCardId,
                address_id: selectedAddressId,
                daily_limit: parseFloat(document.getElementById("dailyLimit").value) || 500,
                require_pin: document.getElementById("requirePin").classList.contains("active") ? 1 : 0,
                auto_apply_credits: document.getElementById("autoCredits").classList.contains("active") ? 1 : 0
            };
            
            const res = await fetch("/mercado/api/quick-buy.php", {
                method: "POST",
