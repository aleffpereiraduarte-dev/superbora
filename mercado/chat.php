<?php
/**
 * 🛒 ONEMUNDO MERCADO - CHAT
 * Chat com suporte ou mercado
 */
...
$customer_name = $customer['firstname'] ?? 'Cliente';
...
$chat_title = $order_id ? "Pedido #$order_id" : "Suporte";

// Mensagens de exemplo (em produção viriam do banco)
$messages = [
    ['from' => 'system', 'text' => 'Olá! Como posso ajudar você hoje?', 'time' => date('H:i', strtotime('-5 minutes'))],
];

if ($order_id) {
    $messages = [
        ['from' => 'system', 'text' => "Olá! Estou aqui para ajudar com seu pedido #$order_id.", 'time' => date('H:i', strtotime('-5 minutes'))],
        ['from' => 'system', 'text' => 'Posso verificar o status, alterar itens ou resolver qualquer dúvida. Como posso ajudar?', 'time' => date('H:i', strtotime('-4 minutes'))],
    ];
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Chat - Mercado OneMundo</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@400;500;600;700;800&display=swap" rel="stylesheet">
    <style>
    :root{--black:#000;--white:#fff;--gray-50:#fafafa;--gray-100:#f4f4f5;--gray-200:#e4e4e7;--gray-300:#d4d4d8;--gray-400:#a1a1aa;--gray-500:#71717a;--gray-600:#52525b;--gray-700:#3f3f46;--gray-800:#27272a;--gray-900:#18181b;--brand-50:#ecfdf5;--brand-100:#d1fae5;--brand-400:#34d399;--brand-500:#10b981;--brand-600:#059669;--rose:#f43f5e;--gradient-brand:linear-gradient(135deg,#10b981 0%,#059669 100%);--shadow-sm:0 2px 4px rgba(0,0,0,0.04);--shadow-lg:0 12px 24px rgba(0,0,0,0.06);--radius-md:10px;--radius-lg:14px;--radius-xl:18px;--radius-2xl:24px;--radius-full:9999px;--font:'Outfit',-apple-system,sans-serif;--ease:cubic-bezier(0.4,0,0.2,1);--safe-top:env(safe-area-inset-top,0px);--safe-bottom:env(safe-area-inset-bottom,0px)}
    *,*::before,*::after{margin:0;padding:0;box-sizing:border-box}
    html,body{height:100%}
    html{-webkit-text-size-adjust:100%;-webkit-tap-highlight-color:transparent}
    body{font-family:var(--font);background:var(--gray-100);color:var(--gray-900);line-height:1.5;-webkit-font-smoothing:antialiased;overflow:hidden;display:flex;flex-direction:column}
    a{text-decoration:none;color:inherit}button{font-family:inherit;cursor:pointer;border:none;background:none}::-webkit-scrollbar{display:none}*{scrollbar-width:none}
    @keyframes fadeIn{from{opacity:0;transform:translateY(10px)}to{opacity:1;transform:translateY(0)}}
    @keyframes typing{0%,60%,100%{transform:translateY(0)}30%{transform:translateY(-4px)}}
    .header{background:var(--gray-900);color:var(--white);padding:0 12px;padding-top:var(--safe-top);flex-shrink:0}@media(min-width:640px){.header{padding:0 16px;padding-top:var(--safe-top)}}
    .headerInner{max-width:900px;margin:0 auto;display:flex;align-items:center;gap:12px;height:56px}@media(min-width:640px){.headerInner{height:64px}}
    .backBtn{width:40px;height:40px;display:flex;align-items:center;justify-content:center;border-radius:var(--radius-md);color:var(--gray-400);flex-shrink:0}.backBtn:active{background:var(--gray-800)}
    .chatInfo{flex:1;display:flex;align-items:center;gap:10px}
    .chatAvatar{width:40px;height:40px;background:var(--gradient-brand);border-radius:var(--radius-full);display:flex;align-items:center;justify-content:center;font-size:18px}
    .chatMeta h1{font-size:15px;font-weight:700}@media(min-width:640px){.chatMeta h1{font-size:16px}}.chatMeta span{font-size:12px;color:var(--brand-400);display:flex;align-items:center;gap:4px}.chatMeta span::before{content:'';width:6px;height:6px;background:var(--brand-400);border-radius:50%}
    .messages{flex:1;overflow-y:auto;padding:16px 12px;display:flex;flex-direction:column;gap:12px}@media(min-width:640px){.messages{padding:20px 16px;gap:16px}}
    .messagesInner{max-width:900px;margin:0 auto;width:100%;display:flex;flex-direction:column;gap:12px}
    .message{display:flex;gap:8px;animation:fadeIn .3s var(--ease)}
    .message.user{flex-direction:row-reverse}
    .msgAvatar{width:32px;height:32px;border-radius:var(--radius-full);display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0;background:var(--gray-200)}
    .message.system .msgAvatar{background:var(--gradient-brand)}
    .msgBubble{max-width:75%;padding:12px 16px;border-radius:var(--radius-xl);font-size:14px;line-height:1.5}@media(min-width:640px){.msgBubble{font-size:15px;padding:14px 18px}}
    .message.system .msgBubble{background:var(--white);color:var(--gray-800);border-bottom-left-radius:4px}
    .message.user .msgBubble{background:var(--gray-900);color:var(--white);border-bottom-right-radius:4px}
    .msgTime{font-size:10px;color:var(--gray-400);margin-top:4px;text-align:right}
    .message.system .msgTime{text-align:left}
    .typing{display:flex;gap:4px;padding:12px 16px;background:var(--white);border-radius:var(--radius-xl);width:fit-content}
    .typing span{width:8px;height:8px;background:var(--gray-300);border-radius:50%;animation:typing 1.4s infinite}.typing span:nth-child(2){animation-delay:.2s}.typing span:nth-child(3){animation-delay:.4s}
    .quickReplies{display:flex;gap:8px;flex-wrap:wrap;padding:12px;background:var(--white);border-top:1px solid var(--gray-100)}@media(min-width:640px){.quickReplies{padding:16px}}
    .quickRepliesInner{max-width:900px;margin:0 auto;width:100%;display:flex;gap:8px;flex-wrap:wrap;overflow-x:auto}
    .quickBtn{padding:8px 14px;background:var(--gray-100);border-radius:var(--radius-full);font-size:13px;font-weight:600;color:var(--gray-600);white-space:nowrap;flex-shrink:0}.quickBtn:active{background:var(--gray-200)}
    .inputBar{background:var(--white);border-top:1px solid var(--gray-100);padding:12px;padding-bottom:calc(12px + var(--safe-bottom));flex-shrink:0}@media(min-width:640px){.inputBar{padding:16px;padding-bottom:calc(16px + var(--safe-bottom))}}
    .inputInner{max-width:900px;margin:0 auto;display:flex;gap:10px;align-items:flex-end}
    .inputWrap{flex:1;background:var(--gray-100);border-radius:var(--radius-xl);padding:4px;display:flex;align-items:flex-end;gap:8px}
    .inputField{flex:1;padding:10px 12px;border:none;background:transparent;font-size:15px;font-family:var(--font);color:var(--gray-900);resize:none;max-height:120px;line-height:1.4}.inputField::placeholder{color:var(--gray-400)}.inputField:focus{outline:none}
    .attachBtn{width:40px;height:40px;display:flex;align-items:center;justify-content:center;color:var(--gray-400);border-radius:var(--radius-md);flex-shrink:0}.attachBtn:active{background:var(--gray-200)}
    .sendBtn{width:44px;height:44px;background:var(--gradient-brand);border-radius:var(--radius-lg);display:flex;align-items:center;justify-content:center;color:var(--white);flex-shrink:0}.sendBtn:active{opacity:.9}.sendBtn:disabled{background:var(--gray-300)}
    </style>
...
<header class="header">
    <div class="headerInner">
        <a href="<?= $order_id ? "pedido.php?id=$order_id" : 'index.php' ?>" class="backBtn"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M19 12H5M12 19l-7-7 7-7"/></svg></a>
        <div class="chatInfo">
            <div class="chatAvatar">🛒</div>
            <div class="chatMeta">
                <h1><?= htmlspecialchars($chat_title) ?></h1>
                <span>Online</span>
            </div>
        </div>
    </div>
...
<div class="messages" id="messages">
    <div class="messagesInner" id="messagesInner">
        <?php foreach ($messages as $msg): ?>
        <div class="message <?= $msg['from'] ?>">
            <div class="msgAvatar"><?= $msg['from'] === 'user' ? '👤' : '🛒' ?></div>
            <div>
                <div class="msgBubble"><?= htmlspecialchars($msg['text']) ?></div>
...
<div class="quickReplies">
    <div class="quickRepliesInner">
        <?php if ($order_id): ?>
        <button class="quickBtn" onclick="sendQuick('Qual o status do meu pedido?')">Status do pedido</button>
        <button class="quickBtn" onclick="sendQuick('Preciso alterar meu endereço')">Alterar endereço</button>
        <button class="quickBtn" onclick="sendQuick('Quero cancelar o pedido')">Cancelar pedido</button>
        <?php else: ?>
        <button class="quickBtn" onclick="sendQuick('Quero fazer um pedido')">Fazer pedido</button>
        <button class="quickBtn" onclick="sendQuick('Tenho uma dúvida sobre produtos')">Dúvida sobre produtos</button>
        <button class="quickBtn" onclick="sendQuick('Problema com entrega')">Problema com entrega</button>
        <?php endif; ?>
    </div>
</div>
...
<div class="inputBar">
    <div class="inputInner">
        <div class="inputWrap">
            <button class="attachBtn"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg></button>
            <textarea class="inputField" id="inputField" rows="1" placeholder="Digite sua mensagem..." onkeydown="handleKey(event)"></textarea>
        </div>
        <button class="sendBtn" id="sendBtn" onclick="sendMessage()"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg></button>
    </div>
</div>
...
function addMessage(text,from='user'){
    ...
    const html=`<div class="message ${from}"><div class="msgAvatar">${from==='user'?'👤':'🛒'}</div><div><div class="msgBubble">${text}</div><div class="msgTime">${time}</div></div></div>`;
    messagesEl.insertAdjacentHTML('beforeend',html);
    scrollToBottom();
}

function showTyping(){
    const html='<div class="message system" id="typing"><div class="msgAvatar">🛒</div><div class="typing"><span></span><span></span><span></span></div></div>';
    messagesEl.insertAdjacentHTML('beforeend',html);
    scrollToBottom();
}
...
function sendMessage(){
    ...
    setTimeout(()=>{
        ...
        const responses=[
            ...
            'Certo, estou analisando sua solicitação.',
            'Perfeito! Um momento enquanto consulto as informações.',
            'Obrigado pelo contato! Já estou verificando.'
        ];
