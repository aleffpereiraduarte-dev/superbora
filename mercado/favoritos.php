...
if (isset($_GET["remove"])) {
    $pdo->prepare("DELETE FROM om_customer_favorites WHERE customer_id = ? AND product_id = ?")->execute([$customer_id, $_GET["remove"]]);
    header("Location: favoritos.php"); exit;
}
$stmt = $pdo->prepare("SELECT f.*, p.image, pd.name, p.price, (SELECT price FROM oc_product_special WHERE product_id = p.product_id AND (date_end IS NULL OR date_end > NOW()) ORDER BY priority LIMIT 1) as special FROM om_customer_favorites f JOIN oc_product p ON f.product_id = p.product_id JOIN oc_product_description pd ON p.product_id = pd.product_id AND pd.language_id = 2 WHERE f.customer_id = ? ORDER BY f.created_at DESC");
...
<div class="card" style="margin:0;padding:16px">
    <div style="height:120px;background:var(--gray-100);border-radius:12px;display:flex;align-items:center;justify-content:center;margin-bottom:12px;position:relative">
        <?php if ($f["image"]): ?><img src="/image/<?= $f["image"] ?>" style="max-width:90%;max-height:90%;object-fit:contain"><?php else: ?>📦<?php endif; ?>
        <button onclick="location.href='?remove=<?= $f["product_id"] ?>'" style="position:absolute;top:8px;right:8px;width:28px;height:28px;border-radius:50%;background:white;border:none;cursor:pointer;font-size:14px">×</button>
    </div>
    ...
    <button class="btn btn-primary" style="width:100%;justify-content:center;padding:10px" onclick="addCart(<?= $f["product_id"] ?>)">+ Carrinho</button>
</div>
<?php endforeach; ?>
</div>
<?php endif; ?>
</main>
<script>function addCart(id){alert("✅ Adicionado!");}</script>
