...
$promocoes_ativas = [
    ['id' => 1, 'tipo' => 'multiplicador', 'titulo' => 'Happy Hour 2X', 'emoji' => '🔥', 'desc' => 'Ganhe o DOBRO em todas as entregas', 'validade' => '18:00 - 21:00 hoje', 'tempo_restante' => '2h 34min', 'cor' => '#EF4444'],
    ['id' => 2, 'tipo' => 'bonus', 'titulo' => 'Bônus de R$50', 'emoji' => '⚡', 'desc' => 'Complete 10 pedidos e ganhe R$50 extra', 'progresso' => 7, 'meta' => 10, 'validade' => 'Até domingo', 'cor' => '#F59E0B'],
    ['id' => 3, 'tipo' => 'zona', 'titulo' => 'Zona Premium: Jardins', 'emoji' => '📍', 'desc' => '+R$8 por entrega na região', 'validade' => 'Hoje até 22:00', 'cor' => '#8B5CF6'],
];

$promocoes_futuras = [
    ['titulo' => 'Especial Natal', 'emoji' => '🎄', 'desc' => '3X em todas as entregas + bônus de R$100', 'validade' => '24-25 Dez', 'cor' => '#00D26A'],
    ['titulo' => 'Desafio Semanal', 'emoji' => '🏆', 'desc' => '30 entregas = R$150 + Badge exclusivo', 'validade' => 'Segunda', 'cor' => '#3B82F6'],
];

$historico = [
    ['titulo' => 'Bônus 15 entregas', 'valor' => 75, 'data' => '10/12/2024'],
    ['titulo' => 'Happy Hour 2X', 'valor' => 45, 'data' => '08/12/2024'],
    ['titulo' => 'Zona Premium', 'valor' => 32, 'data' => '05/12/2024'],
];
?>
<!DOCTYPE html>
<html lang="pt-BR">
...
<style>
:root{--bg:#fff;--bg2:#f5f5f5;--card:#fff;--border:#e0e0e0;--txt:#1a1a1a;--txt2:#666;--txt3:#999;--brand:#00a650;--brand-lt:#e8f5e9;--safe-t:env(safe-area-inset-top);--safe-b:env(safe-area-inset-bottom)}
*{margin:0;padding:0;box-sizing:border-box;-webkit-tap-highlight-color:transparent}
body{font-family:-apple-system,system-ui,sans-serif;background:var(--bg2);color:var(--txt);min-height:100vh}
.app{max-width:430px;margin:0 auto;background:var(--bg2);min-height:100vh}
...
.main{padding:16px;padding-bottom:calc(40px + var(--safe-b))}

.earnings-card{background:linear-gradient(135deg,var(--brand),#00874a);border-radius:20px;padding:24px;text-align:center;color:#fff;margin-bottom:20px}
.earnings-label{font-size:13px;opacity:.85;margin-bottom:4px}
.earnings-value{font-size:38px;font-weight:800}
.earnings-period{font-size:12px;opacity:.7;margin-top:4px}

.section{margin-bottom:24px}
.section-title{font-size:16px;font-weight:700;margin-bottom:12px;display:flex;align-items:center;gap:8px}
.section-badge{padding:4px 10px;background:var(--brand);border-radius:10px;font-size:11px;font-weight:700;color:#fff}

.promo-card{background:var(--card);border-radius:16px;overflow:hidden;margin-bottom:12px;box-shadow:0 2px 8px rgba(0,0,0,.06)}
.promo-header{padding:16px;position:relative}
.promo-live{position:absolute;top:12px;right:12px;padding:4px 10px;background:#EF4444;border-radius:12px;font-size:10px;font-weight:700;color:#fff;display:flex;align-items:center;gap:4px}
.promo-live::before{content:'';width:6px;height:6px;background:#fff;border-radius:50%;animation:blink 1s infinite}
@keyframes blink{50%{opacity:.4}}
.promo-icon{width:48px;height:48px;border-radius:14px;display:flex;align-items:center;justify-content:center;font-size:24px;margin-bottom:12px}
.promo-title{font-size:17px;font-weight:700;margin-bottom:4px}
.promo-desc{font-size:13px;color:var(--txt2);margin-bottom:10px}
.promo-time{display:flex;align-items:center;gap:6px;font-size:12px;color:var(--txt3)}
.promo-time svg{width:14px;height:14px}
.promo-timer{font-weight:600;color:var(--txt)}

.promo-progress{padding:12px 16px 16px;border-top:1px solid var(--border)}
.progress-bar{height:8px;background:var(--bg2);border-radius:4px;overflow:hidden;margin-bottom:8px}
.progress-fill{height:100%;border-radius:4px;transition:width .3s}
.progress-text{display:flex;justify-content:space-between;font-size:12px;color:var(--txt2)}
.progress-text strong{color:var(--txt)}

.future-card{background:var(--card);border-radius:14px;padding:14px;margin-bottom:10px;display:flex;align-items:center;gap:12px;box-shadow:0 2px 6px rgba(0,0,0,.04)}
.future-icon{width:44px;height:44px;border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:22px}
.future-info{flex:1}
.future-title{font-size:14px;font-weight:600}
.future-desc{font-size:12px;color:var(--txt2)}
.future-date{padding:5px 10px;background:var(--bg2);border-radius:8px;font-size:11px;color:var(--txt2);font-weight:500}

.history-card{background:var(--card);border-radius:14px;overflow:hidden;box-shadow:0 2px 6px rgba(0,0,0,.04)}
.history-item{display:flex;align-items:center;gap:12px;padding:14px 16px;border-bottom:1px solid var(--border)}
.history-item:last-child{border-bottom:0}
.history-icon{width:38px;height:38px;background:var(--brand-lt);border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:18px}
.history-info{flex:1}
.history-title{font-size:13px;font-weight:600}
.history-date{font-size:11px;color:var(--txt3)}
.history-value{font-size:15px;font-weight:700;color:var(--brand)}

.notify-card{background:var(--card);border-radius:14px;padding:16px;display:flex;align-items:center;gap:14px;margin-top:16px;box-shadow:0 2px 6px rgba(0,0,0,.04)}
.notify-icon{font-size:28px}
.notify-text{flex:1;font-size:13px;color:var(--txt2)}
.notify-toggle{width:50px;height:28px;background:var(--brand);border-radius:14px;position:relative;cursor:pointer}
.notify-toggle::after{content:'';position:absolute;top:2px;right:2px;width:24px;height:24px;background:#fff;border-radius:50%;transition:all .2s}
.notify-toggle.off{background:var(--border)}
.notify-toggle.off::after{right:auto;left:2px}
</style>
...
<div class="app">
    ...
    <main class="main">
        <div class="earnings-card">
            <div class="earnings-label">Ganhos com promoções</div>
            <div class="earnings-value">R$ 152,00</div>
            <div class="earnings-period">Este mês</div>
        </div>

        <div class="section">
            <h2 class="section-title">Ativas Agora <span class="section-badge"><?= count($promocoes_ativas) ?></span></h2>

            <?php foreach ($promocoes_ativas as $promo): ?>
            <div class="promo-card">
                <div class="promo-header" style="background:linear-gradient(135deg,<?= $promo['cor'] ?>15,transparent)">
                    <span class="promo-live">AO VIVO</span>
                    <div class="promo-icon" style="background:<?= $promo['cor'] ?>20"><?= $promo['emoji'] ?></div>
                    ...
                    <div class="promo-time">
                        ...
                        <?= $promo['validade'] ?>
                        <?php if (isset($promo['tempo_restante'])): ?>
                        <span class="promo-timer">• <?= $promo['tempo_restante'] ?></span>
                        <?php endif; ?>
                    </div>
                </div>
                <?php if (isset($promo['progresso'])): ?>
                <div class="promo-progress">
                    <div class="progress-bar">
                        <div class="progress-fill" style="width:<?= ($promo['progresso'] / $promo['meta']) * 100 ?>%;background:<?= $promo['cor'] ?>"></div>
                    </div>
                    <div class="progress-text">
                        <span><strong><?= $promo['progresso'] ?></strong> de <?= $promo['meta'] ?> pedidos</span>
                        <span>Faltam <?= $promo['meta'] - $promo['progresso'] ?></span>
                    </div>
                </div>
                <?php endif; ?>
            </div>
        ...
        <div class="section">
            ...
            <?php foreach ($promocoes_futuras as $promo): ?>
            <div class="future-card">
                <div class="future-icon" style="background:<?= $promo['cor'] ?>20"><?= $promo['emoji'] ?></div>
                <div class="future-info">
                    <div class="future-title"><?= $promo['titulo'] ?></div>
                    <div class="future-desc"><?= $promo['desc'] ?></div>
                </div>
                <div class="future-date"><?= $promo['validade'] ?></div>
            </div>
            <?php endforeach; ?>
        </div>

        <div class="section">
            <h2 class="section-title">📊 Histórico de Bônus</h2>
            <div class="history-card">
                ...
                <div class="history-item">
                    ...
                    <div class="history-value">+R$ <?= number_format($h['valor'], 2, ',', '.') ?></div>
                </div>
                <?php endforeach; ?>
            </div>
        </div>
        ...
        <div class="notify-card">
            ...
            <span class="notify-text">Receber alertas de novas promoções</span>
            <div class="notify-toggle" onclick="this.classList.toggle('off')"></div>
        </div>
    </main>
</div>
