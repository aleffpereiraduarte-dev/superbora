...
$hoje = date('w'); // 0=Dom, 6=Sab

// Agenda da semana (simulada)
$agenda = [
    0 => ['ativo' => false, 'inicio' => '', 'fim' => ''],
    1 => ['ativo' => true, 'inicio' => '08:00', 'fim' => '18:00'],
    2 => ['ativo' => true, 'inicio' => '08:00', 'fim' => '18:00'],
    3 => ['ativo' => true, 'inicio' => '10:00', 'fim' => '20:00'],
    4 => ['ativo' => true, 'inicio' => '08:00', 'fim' => '18:00'],
    5 => ['ativo' => true, 'inicio' => '08:00', 'fim' => '22:00'],
    6 => ['ativo' => true, 'inicio' => '09:00', 'fim' => '15:00'],
];
...
for ($i = 0; $i < 7; $i++) {
    ...
    $proxDias[] = [
        'data' => date('d/m', $timestamp),
        'dia' => $diasSemana[$diaSemana],
        'diaSemana' => $diaSemana,
        'hoje' => $i === 0,
        'config' => $agenda[$diaSemana]
    ];
}
?>
<!DOCTYPE html>
...
<style>
:root{--bg:#fff;--bg2:#f5f5f5;--card:#fff;--border:#e0e0e0;--txt:#1a1a1a;--txt2:#666;--txt3:#999;--brand:#00a650;--brand-lt:#e8f5e9;--red:#ef4444;--red-lt:#fee2e2;--blue:#1a73e8;--safe-t:env(safe-area-inset-top);--safe-b:env(safe-area-inset-bottom)}
...
.main{padding:16px;padding-bottom:calc(100px + var(--safe-b))}

.info-card{background:var(--blue);border-radius:16px;padding:16px;color:#fff;margin-bottom:20px;display:flex;align-items:center;gap:14px}
.info-icon{width:44px;height:44px;background:rgba(255,255,255,.2);border-radius:12px;display:flex;align-items:center;justify-content:center;font-size:22px}
.info-text{flex:1}
.info-text h3{font-size:15px;font-weight:600}
.info-text p{font-size:12px;opacity:.85;margin-top:2px}

.section{margin-bottom:24px}
.section-title{font-size:14px;font-weight:600;color:var(--txt2);margin-bottom:12px;padding-left:4px}

.week-view{display:flex;gap:8px;overflow-x:auto;padding-bottom:8px;margin-bottom:20px}
.day-card{flex:0 0 auto;width:70px;padding:12px 8px;background:var(--card);border-radius:14px;text-align:center;border:2px solid transparent;cursor:pointer}
.day-card.hoje{border-color:var(--brand);background:var(--brand-lt)}
.day-card.inativo{opacity:.5}
.day-name{font-size:11px;color:var(--txt2);font-weight:600;text-transform:uppercase}
.day-date{font-size:18px;font-weight:700;margin:6px 0}
.day-status{font-size:10px;padding:3px 6px;border-radius:6px;font-weight:600}
.day-card.ativo .day-status{background:var(--brand-lt);color:var(--brand)}
.day-card.inativo .day-status{background:var(--bg2);color:var(--txt3)}

.schedule-card{background:var(--card);border-radius:16px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.06)}
.schedule-item{display:flex;align-items:center;gap:14px;padding:16px;border-bottom:1px solid var(--border)}
.schedule-item:last-child{border-bottom:0}
.schedule-day{width:50px;text-align:center}
.schedule-day-name{font-size:12px;color:var(--txt2);font-weight:600}
.schedule-day-num{font-size:11px;color:var(--txt3)}
.schedule-time{flex:1}
.schedule-hours{font-size:15px;font-weight:600}
.schedule-duration{font-size:12px;color:var(--txt2)}
.schedule-toggle{width:50px;height:28px;border-radius:14px;position:relative;cursor:pointer;transition:background .2s}
.schedule-toggle.on{background:var(--brand)}
.schedule-toggle.off{background:var(--border)}
.schedule-toggle::after{content:'';position:absolute;top:2px;width:24px;height:24px;background:#fff;border-radius:50%;transition:all .2s;box-shadow:0 2px 4px rgba(0,0,0,.2)}
.schedule-toggle.on::after{right:2px}
.schedule-toggle.off::after{left:2px}

.edit-btn{display:flex;align-items:center;justify-content:center;gap:8px;width:100%;padding:14px;background:var(--bg2);border:0;border-radius:12px;font-size:14px;font-weight:600;color:var(--txt);cursor:pointer;margin-top:16px}
.edit-btn svg{width:18px;height:18px}

.tips-card{background:var(--card);border-radius:16px;padding:16px;margin-top:20px}
.tips-title{font-size:14px;font-weight:600;margin-bottom:12px;display:flex;align-items:center;gap:8px}
.tip-item{display:flex;align-items:flex-start;gap:10px;margin-bottom:10px;font-size:13px;color:var(--txt2)}
.tip-item:last-child{margin-bottom:0}
.tip-icon{font-size:16px}

.bottom-bar{position:fixed;bottom:0;left:50%;transform:translateX(-50%);width:100%;max-width:430px;background:var(--bg);border-top:1px solid var(--border);padding:12px 16px calc(12px + var(--safe-b))}
.save-btn{width:100%;padding:14px;background:var(--brand);color:#fff;border:0;border-radius:12px;font-size:15px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px}
    ...
    <main class="main">
        <div class="info-card">
            ...
            <div class="info-text">
                ...
                <p>Você só receberá pedidos nos horários configurados</p>
            </div>
        </div>

        <!-- Visualização da semana -->
        <div class="week-view">
            <?php foreach ($proxDias as $d): ?>
            <div class="day-card <?= $d['hoje'] ? 'hoje' : '' ?> <?= $d['config']['ativo'] ? 'ativo' : 'inativo' ?>">
                <div class="day-name"><?= $d['dia'] ?></div>
                <div class="day-date"><?= explode('/', $d['data'])[0] ?></div>
                <div class="day-status"><?= $d['config']['ativo'] ? $d['config']['inicio'] : 'Folga' ?></div>
            </div>
            <?php endforeach; ?>
        </div>
        ...
        <div class="section">
            <div class="section-title">Horários da Semana</div>
            <div class="schedule-card">
                <?php foreach ($diasSemana as $i => $dia): ?>
                <div class="schedule-item">
                    <div class="schedule-day">
                        <div class="schedule-day-name"><?= $dia ?></div>
                    </div>
                    <div class="schedule-time">
                        <?php if ($agenda[$i]['ativo']): ?>
                        <div class="schedule-hours"><?= $agenda[$i]['inicio'] ?> - <?= $agenda[$i]['fim'] ?></div>
                        <div class="schedule-duration">
                            <?php 
                            $h1 = strtotime($agenda[$i]['inicio']);
                            $h2 = strtotime($agenda[$i]['fim']);
                            $diff = ($h2 - $h1) / 3600;
                            echo $diff . ' horas';
                            ?>
                        </div>
                        <?php else: ?>
                        <div class="schedule-hours" style="color:var(--txt3)">Folga</div>
                        <div class="schedule-duration">Não disponível</div>
                        <?php endif; ?>
                    </div>
                    <div class="schedule-toggle <?= $agenda[$i]['ativo'] ? 'on' : 'off' ?>" onclick="toggleDay(this, <?= $i ?>)"></div>
                </div>
                <?php endforeach; ?>
            </div>
        ...
        <div class="tips-card">
            ...
            <div class="tip-item">
                ...
                <span>Sexta e sábado à noite têm mais pedidos e bônus maiores</span>
            </div>
            <div class="tip-item">
                <span class="tip-icon">⏰</span>
                <span>Horário de almoço (11h-14h) tem alta demanda</span>
            </div>
            <div class="tip-item">
                <span class="tip-icon">📍</span>
                <span>Fique próximo de shoppings e centros comerciais</span>
            </div>
        </div>
    </main>
...
function editSchedule() {
    alert('⏰ Editar Horários\n\nEm breve você poderá personalizar os horários de cada dia individualmente.\n\nPor enquanto, use os toggles para ativar/desativar os dias.');
}
...
function saveSchedule() {
    ...
    alert('✅ Agenda salva com sucesso!\n\nVocê receberá pedidos apenas nos horários configurados.');
