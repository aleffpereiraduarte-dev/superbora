...
$orderData = [
    ...
    'client_photo' => 'https://randomuser.me/api/portraits/women/65.jpg',
    'client_phone' => '11999001122',
    'address' => 'Rua Augusta, 1200 - Apto 45',
    'password' => '7842', // Senha de 4 dígitos
    'earnings' => 12.00
];
?>
<!DOCTYPE html>
...
<style>
:root{--bg:#fff;--bg2:#f5f5f5;--border:#e0e0e0;--txt:#1a1a1a;--txt2:#666;--txt3:#999;--brand:#00a650;--brand-lt:#e8f5e9;--blue:#1a73e8;--orange:#f57c00;--orange-lt:#fff3e0;--red:#d32f2f;--safe-t:env(safe-area-inset-top);--safe-b:env(safe-area-inset-bottom)}
...
.client-card{display:flex;align-items:center;gap:14px;padding:16px;background:var(--bg2);border-radius:14px;margin-bottom:20px}
.client-photo{width:56px;height:56px;border-radius:50%;object-fit:cover;border:3px solid var(--brand)}
.client-info{flex:1}
.client-name{font-size:16px;font-weight:600}
.client-address{font-size:12px;color:var(--txt2);margin-top:2px}
.client-actions{display:flex;gap:8px}
.client-btn{width:40px;height:40px;border-radius:10px;display:flex;align-items:center;justify-content:center;border:0;cursor:pointer}
.client-btn.chat{background:var(--brand);color:#fff}
.client-btn.call{background:var(--blue);color:#fff}
.client-btn svg{width:20px;height:20px}
.confirm-section{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:20px}
.confirm-icon{width:80px;height:80px;background:var(--brand-lt);border-radius:50%;display:flex;align-items:center;justify-content:center;margin-bottom:20px}
.confirm-icon svg{width:40px;height:40px;color:var(--brand)}
.confirm-title{font-size:20px;font-weight:700;margin-bottom:8px}
.confirm-subtitle{font-size:14px;color:var(--txt2);margin-bottom:24px;line-height:1.5}
.confirm-options{width:100%;max-width:320px}
.option-btn{width:100%;padding:16px;border:2px solid var(--border);border-radius:14px;background:var(--bg);margin-bottom:12px;cursor:pointer;display:flex;align-items:center;gap:14px;transition:all .2s}
.option-btn:hover{border-color:var(--brand);background:var(--brand-lt)}
.option-btn.active{border-color:var(--brand);background:var(--brand-lt)}
.option-icon{width:44px;height:44px;background:var(--bg2);border-radius:12px;display:flex;align-items:center;justify-content:center}
.option-btn.active .option-icon{background:var(--brand)}
.option-btn.active .option-icon svg{color:#fff}
.option-icon svg{width:22px;height:22px;color:var(--txt)}
.option-text{flex:1;text-align:left}
.option-text h4{font-size:14px;font-weight:600}
.option-text p{font-size:12px;color:var(--txt2)}
.password-input-section{display:none;margin-top:20px;width:100%;max-width:320px}
.password-input-section.show{display:block}
.password-label{font-size:13px;color:var(--txt2);margin-bottom:10px}
.password-input{width:100%;padding:16px;font-size:24px;font-weight:700;text-align:center;letter-spacing:12px;border:2px solid var(--border);border-radius:12px;background:var(--bg);font-family:monospace}
.password-input:focus{border-color:var(--brand);outline:none}
.scanner-section{display:none;margin-top:20px;width:100%;max-width:320px}
.scanner-section.show{display:block}
.scanner-box{width:100%;aspect-ratio:1;background:#000;border-radius:16px;position:relative;overflow:hidden}
#scanner-video{width:100%;height:100%;object-fit:cover}
.scanner-overlay{position:absolute;inset:0;display:flex;align-items:center;justify-content:center}
.scanner-frame{width:70%;height:70%;border:3px solid var(--brand);border-radius:16px;box-shadow:0 0 0 9999px rgba(0,0,0,.5)}
.scanner-hint{position:absolute;bottom:20px;left:0;right:0;text-align:center;color:#fff;font-size:12px}
.bottom{padding:16px;padding-bottom:calc(16px + var(--safe-b));border-top:1px solid var(--border)}
.btn{width:100%;padding:16px;border:0;border-radius:14px;font-size:15px;font-weight:600;display:flex;align-items:center;justify-content:center;gap:8px;cursor:pointer;font-family:inherit}
.btn.primary{background:var(--brand);color:#fff}
.btn:disabled{opacity:.5}
.btn svg{width:20px;height:20px}
.success-modal{display:none;position:fixed;inset:0;background:var(--brand);z-index:200;flex-direction:column;align-items:center;justify-content:center;color:#fff;text-align:center;padding:24px}
.success-modal.show{display:flex}
.success-icon{width:100px;height:100px;background:rgba(255,255,255,.2);border-radius:50%;display:flex;align-items:center;justify-content:center;margin-bottom:24px;animation:pop .5s}
@keyframes pop{0%{transform:scale(0)}100%{transform:scale(1)}}
.success-icon svg{width:50px;height:50px}
.success-title{font-size:26px;font-weight:700;margin-bottom:8px}
.success-subtitle{font-size:15px;opacity:.9;margin-bottom:32px}
.success-earnings{background:rgba(255,255,255,.2);border-radius:16px;padding:20px;margin-bottom:32px}
.success-earnings small{font-size:12px;opacity:.85}
.success-earnings strong{display:block;font-size:36px;font-weight:700}
.success-btn{padding:16px 40px;background:#fff;color:var(--brand);border:0;border-radius:14px;font-size:16px;font-weight:600;cursor:pointer}
.chat-modal{display:none;position:fixed;inset:0;background:var(--bg);z-index:150;flex-direction:column}
...
<div class="app">
    ...
    <div class="main">
        <div class="client-card">
            <img class="client-photo" src="<?= $orderData['client_photo'] ?>">
            <div class="client-info">
                <div class="client-name"><?= $orderData['client_name'] ?></div>
                <div class="client-address"><?= $orderData['address'] ?></div>
            </div>
            <div class="client-actions">
                <button class="client-btn chat" onclick="openChat()"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/></svg></button>
                <button class="client-btn call" onclick="location.href='tel:<?= $orderData['client_phone'] ?>'"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg></button>
            </div>
        </div>
        
        <div class="confirm-section">
            <div class="confirm-icon"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg></div>
            <h2 class="confirm-title">Confirmar Entrega</h2>
            <p class="confirm-subtitle">Peça ao cliente para confirmar o recebimento usando uma das opções abaixo</p>
            
            <div class="confirm-options">
                <div class="option-btn" onclick="selectOption('password')" id="opt-password">
                    <div class="option-icon"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg></div>
                    <div class="option-text">
                        <h4>Senha de 4 dígitos</h4>
                        <p>Cliente fala a senha do pedido</p>
                    </div>
                </div>
                <div class="option-btn" onclick="selectOption('qr')" id="opt-qr">
                    <div class="option-icon"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h4.01M16 20h4M4 12h4m12 0h.01M5 8h2a1 1 0 001-1V5a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1zm12 0h2a1 1 0 001-1V5a1 1 0 00-1-1h-2a1 1 0 00-1 1v2a1 1 0 001 1zM5 20h2a1 1 0 001-1v-2a1 1 0 00-1-1H5a1 1 0 00-1 1v2a1 1 0 001 1z"/></svg></div>
                    <div class="option-text">
                        <h4>Escanear QR Code</h4>
                        <p>Cliente mostra QR no app</p>
                    </div>
                </div>
            </div>
            
            <div class="password-input-section" id="password-section">
                <div class="password-label">Digite a senha informada pelo cliente:</div>
                <input type="tel" class="password-input" id="password-input" maxlength="4" placeholder="0000" oninput="checkPassword()">
            </div>
            ...
            <div class="scanner-section" id="scanner-section">
                <div class="scanner-box">
                    ...
                    <div class="scanner-hint">Aponte para o QR Code do cliente</div>
                </div>
            </div>
        </div>
    </div>
    <div class="bottom">
        <button class="btn primary" id="confirm-btn" disabled onclick="confirmDelivery()"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>Confirmar Entrega</button>
    </div>
</div>

<div class="success-modal" id="success-modal">
    <div class="success-icon"><svg fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"/></svg></div>
    <h2 class="success-title">Entrega Concluída!</h2>
    <p class="success-subtitle">Parabéns! Você finalizou mais uma entrega.</p>
    <div class="success-earnings">
        <small>Você ganhou</small>
        <strong>R$ <?= number_format($orderData['earnings'], 2, ',', '.') ?></strong>
    </div>
    <button class="success-btn" onclick="location.href='app.php'">Voltar ao Início</button>
</div>
...
<div class="chat-modal" id="chat-modal">
    <div class="chat-header">
        ...
        <img class="chat-photo" src="<?= $orderData['client_photo'] ?>">
        <div class="chat-info"><h4><?= $orderData['client_name'] ?></h4><p>Cliente • Online</p></div>
    </div>
...
const correctPassword = '<?= $orderData['password'] ?>';
let selectedOption = null, verified = false, msgs = [];

function selectOption(opt) {
    selectedOption = opt;
    document.getElementById('opt-password').classList.toggle('active', opt === 'password');
    document.getElementById('opt-qr').classList.toggle('active', opt === 'qr');
    document.getElementById('password-section').classList.toggle('show', opt === 'password');
    document.getElementById('scanner-section').classList.toggle('show', opt === 'qr');
...
function checkPassword() {
    ...
    if (input.length === 4) {
        if (input === correctPassword) {
            ...
            document.getElementById('password-input').style.borderColor = 'var(--brand)';
...
async function startScanner() {
    try {
        ...
        // Simular leitura de QR após 3 segundos
        setTimeout(() => {
...
function confirmDelivery() {
    if (!verified) return;
    fetch('api/complete-delivery.php', {
        method: 'POST',
