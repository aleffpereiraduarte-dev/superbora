<?php
/**
 * Smart Auth - Verificar Telefone
 *
 * POST /api/smart-auth/verify-phone.php
 * {
 *   "customer_id": 123,
 *   "code": "123456"
 * }
...
try {
    ...
    $stmt = $pdo->prepare("
        ...
        WHERE customer_id = ? AND type = 'phone' AND code = ?
    ...
    // Verificar se email também está verificado
    $stmt = $pdo->prepare("
        SELECT verified_at FROM om_verification_codes
        WHERE customer_id = ? AND type = 'email'
        ORDER BY created_at DESC
        LIMIT 1
    ");
    $stmt->execute([$customerId]);
    $emailVerification = $stmt->fetch(PDO::FETCH_ASSOC);

    $emailVerified = !empty($emailVerification['verified_at']);
    ...
    echo json_encode([
        ...
        'account_active' => $emailVerified,
        'next_step' => $emailVerified ? 'login' : 'verify_email'
    ]);
